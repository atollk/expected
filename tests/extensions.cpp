...
TEST_CASE("Map extensions", "[extensions.map]") {
  ...
  {
    const atl::expected<int, int> e = 21;
    auto ret = e.map(mul2);
    REQUIRE(ret);
    REQUIRE(*ret == 42);
  }
  ...
  {
    atl::expected<int, int> e(atl::unexpect, 21);
    auto ret = e.map(mul2);
    REQUIRE(!ret);
  ...
  {
    const atl::expected<int, int> e(atl::unexpect, 21);
    auto ret = e.map(mul2);
    REQUIRE(!ret);
    REQUIRE(ret.error() == 21);
  }
...

  {
    ...
    REQUIRE(ret);
    STATIC_REQUIRE(
        (std::is_same<decltype(ret), atl::expected<void, int>>::value));
  }
...
TEST_CASE("And then extensions", "[extensions.and_then]") {
  auto succeed = [](int a) { return atl::expected<int, int>(21 * 2); };
  auto fail = [](int a) { return atl::expected<int, int>(atl::unexpect, 17); };
...
TEST_CASE("or_else", "[extensions.or_else]") {
  ...
  auto succeed = [](int a) { return atl::expected<int, int>(21 * 2); };
  auto succeedptr = [](eptr e) { return atl::expected<int,eptr>(21*2);};
  auto fail =    [](int a) { return atl::expected<int,int>(atl::unexpect, 17);};
  auto efail =   [](eptr e) { *e = 17;return atl::expected<int,eptr>(atl::unexpect, std::move(e));};
  auto failptr = [](eptr e) { return atl::expected<int,eptr>(atl::unexpect, std::move(e));};
  auto failvoid = [](int) {};
  ...
  {
    atl::expected<int, eptr> e(atl::unexpect, make_u_int(21));
    auto ret = std::move(e).or_else(succeedptr);
    REQUIRE(ret);
    REQUIRE(*ret == 42);
  }
...
TEST_CASE("14", "[issue.14]") {
    auto res = atl::expected<S,F>{atl::unexpect, F{}};
